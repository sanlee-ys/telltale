...
type State struct {
	...
	// FlowHop / FlowSteps / FlowVendor describe the chain driving the room, and
	// exist because the room started dispatching without a keystroke.
	//
	// Every other turn here is something the user pressed enter on. A /flow
	// chain advances itself, so between hops the room sends a brief nobody
	// typed to a seat nobody selected — and with three of four columns idle it
	// looks the same as a room doing something on its own. FlowSteps == 0 means
	// no chain, which is the ordinary case and renders nothing.

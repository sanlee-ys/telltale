...
func writeDetail(v model.VendorID, asking bool) string {
	...
	case model.VendorCodex, model.VendorGrok, model.VendorAntigravity:
		...
		// (design.md §9.54), each on a documentation read rather than a live
...
// (design.md §9.54) and nothing here has watched one run, so every live entry
...
// exactly one word, and the checklist in §9.54 is the price of flipping it.
//
...
// (2026-09-02, design.md §9.54). The codex app-server seat routes
...
// change, this list is the one line to change and §9.54 is the record to
// update.
	...
	case model.VendorCodex:
		if windows {
			return SandboxClaim{
				...
				// The seat moved to `codex app-server` on 2026-09-02 (§9.54)

...
func header(st State, lay Layout, sty Styles, g Glyphs) string {
	...
	if st.Write {
		...
		// marker that failed to render, and the reader cannot tell those apart.
	...
	// the user did not press enter on, which is why it is not muted like the
	// counts beside it: three idle columns and a brief nobody typed is
	// otherwise indistinguishable from the room acting on its own.
	if st.FlowSteps > 0 {

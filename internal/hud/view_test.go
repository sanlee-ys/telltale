...
func goldenCases() []goldenCase {
	return []goldenCase{
		...
		// nothing that is.
		{name: "usage-fleet", state: func() State { return usageFleetState(120, 25) }},
		...
		{name: "usage-floor", state: func() State { return usageFleetState(60, 25) }},
		...
		{name: "usage-ascii", ascii: true, state: func() State { return usageFleetState(120, 25) }},
		...
		{name: "usage-stale-relay", state: func() State { return usageStaleRelayState(120, 24) }},
		...
		{name: "usage-stale-ascii", ascii: true, state: func() State { return usageStaleRelayState(120, 24) }},
...
func TestUsageNeverRendersAQuotaLessVendorAsZero(t *testing.T) {
	g := UnicodeGlyphs()
	for _, line := range usageBodyOf(t, usageFleetState(120, 25), g) {
...
func TestUsageTotalNeverPrintsWithoutItsWindow(t *testing.T) {
	for _, w := range []int{200, 120, 100, 80, 72, 60} {
		var spent string
		for _, line := range usageBodyOf(t, usageFleetState(w, 25), UnicodeGlyphs()) {
			if strings.Contains(line, "spent") {
				spent = line
			}
		}
...
func TestUsageKeepsTheKindsOfAbsenceApart(t *testing.T) {
	body := strings.Join(usageBodyOf(t, usageFleetState(120, 25), UnicodeGlyphs()), "\n")
...
func TestUsageGivesTheGaugeRealRoom(t *testing.T) {
	...
	narrow := strings.Join(usageBodyOf(t, usageFleetState(60, 25), g), "\n")
...
func TestUsageFactsShareOneColumnGridAcrossVendors(t *testing.T) {
	...
	for _, w := range []int{120, 99, 80, 60} {
		...
		for _, line := range usageBodyOf(t, usageFleetState(w, 25), g) {
...
func TestARelayedReadingGetsLouderAsItAges(t *testing.T) {
	aged := func(d time.Duration) State {
		st := usageStaleRelayState(120, 24)
...
func TestNoLineExceedsTheTerminalWidth(t *testing.T) {
	for _, w := range []int{60, 72, 80, 99, 100, 120, 200} {
		...
		out := Render(usageFleetState(w, 25), PlainStyles(), UnicodeGlyphs())
		for i, line := range strings.Split(out, "\n") {
			if n := len([]rune(line)); n > w {

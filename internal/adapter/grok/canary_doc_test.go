...
func TestTheCanaryInventoryMatchesThisAdapter(t *testing.T) {
	...
	if !strings.Contains(doc, canarySummaryInfoID.Name) {
		t.Errorf("design.md §3.10 does not name the %q canary; the inventory has drifted from the adapter", canarySummaryInfoID.Name)
	}
}
